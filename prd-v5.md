...
#### 8.1.1 技术栈选择
* **核心框架**: React Native + Expo SDK 54（跨平台移动端开发）
* **开发语言**: TypeScript
* **状态管理**: React Context API + useReducer（无 Redux）
* **UI组件库**: React Native Paper
* **样式方案**: Tailwind CSS（NativeWind）
* **网络请求**: Axios
* **本地存储**: AsyncStorage（Expo）
* **导航**: React Navigation
* **图片/相机**: Expo ImagePicker、Expo Camera
* **地图集成**: Expo MapView
...
#### 8.2.1 架构设计
* **服务架构**: 单体 Express 应用（Express 5.2.1），模块化路由与中间件
* **API风格**: RESTful API
* **认证机制**: JWT + 刷新令牌
* **数据库**: MongoDB（Mongoose 9 ODM），所有数据统一存储，不使用多数据库混用
* **日志**: morgan 请求日志
* **跨域**: CORS 中间件

#### 8.2.2 AI诊断服务技术要求
* **AI引擎**: 通义千问（阿里云 DashScope）
  - 对话问诊：流式对话，通过 DashScope 大模型 API 生成诊断建议
  - 初诊分析：整合养殖基础信息与临床表现，生成带置信度的初诊结论
  - 混合感染风险评估：基于病原流行规律与症状关联性，识别高风险组合
  - 防控方案生成：按"紧急控制-短期治疗-生物安全"维度输出标准化方案
  - 确诊方案推荐：分层级实验室检测方案（紧急/重要/深入）
  - 学生模拟诊断：生成 AI 诊断结果、评分与反馈
* **百度云图像识别**（可选）：病禽图片辅助识别，结果作为补充输入
* **提示词工程**: 采用结构化提示词模板，详见 2.0.2 节各场景设计
* **流式响应**: AI 对话与报告生成均支持 SSE 流式输出

#### 8.2.3 核心模块
* **用户服务**: 负责用户注册、认证、权限管理
* **AI诊断服务**: 对接通义千问 API，处理诊断请求与流式响应
* **生产数据服务**: 处理养殖数据的 CRUD 和统计分析
* **文件服务**: 处理图片等多媒体文件上传与存储
* **疫情监测服务**: 处理疫情数据聚合与预警逻辑

#### 8.2.4 数据存储设计
* **主数据库**: MongoDB（所有业务数据，含用户、诊断记录、生产数据、订单等）
* **对象存储**: 阿里云 OSS / AWS S3（图片、视频等非结构化文件）
* **数据备份**: 每日全量备份 + 实时增量备份
